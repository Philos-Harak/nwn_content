...
void main()
{
    ...
    //ai_Debug("0e_ch_6_damaged", "18", GetName(oCreature) + " has been damaged by " + GetName(oDamager));
    ...
    if(GetObjectType(oDamager) != OBJECT_TYPE_AREA_OF_EFFECT)
    {
        if(ai_CanIAttack(oCreature) &&
           !ai_GetIsBusy(oCreature) &&
           ai_GetIsInCombat(oCreature)) ai_DoAssociateCombatRound(oCreature);
        return;
    }
