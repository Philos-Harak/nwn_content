...
// add to nw_ch_ac7 OnDeath script of henchman.
void ai_OnDeath(object oCreature, object oKiller);
// Add to nw_c2_default9 OnSpawn script of monsters and
void ai_OnMonsterSpawn(object oCreature, int bIncorporeal);
// Add to nw_ch_ac9 OnSpawn script of henchman.
...
void ai_DoAssociateCombatRound(object oCreature, object oTarget = OBJECT_INVALID)
{
    ...
    if (oNearestEnemy != OBJECT_INVALID || oTarget != OBJECT_INVALID)
    {
        ...
        ai_Debug("0i_actions", "78", "********** " + GetName (oCreature) + " **********");
        ai_Debug("0i_actions", "79", "********** " + sAI + " **********");
    ...
    ai_Debug("0i_actions", "93", GetName (OBJECT_SELF) + "'s combat has ended!");
}
int ai_CanIAttack(object oAssociate)
{
...
int ai_AttemptToByPassLock(object oCreature, object oLocked)
{
    ai_Debug("0i_associates", "124", "oCreature: " + GetName(oCreature) + " oLocked:" + GetName(oLocked));
    ai_Debug("0i_associates", "125", "Trapped?: " + IntToString(GetIsTrapped(oLocked)) +
    ...
    ai_Debug("0i_associates", "143", "sKeyTag: " + sKeyTag);
    if(sKeyTag != "")
    {
        // Do we have the key?
    ...
    ai_Debug("0i_associates", "156", "bCanDo: " + IntToString(bCanDo) +
    ...
    ai_Debug("0i_associates", "167", "bCanDo: " + IntToString(bCanDo) +
           " nSkill:" + IntToString(nSkill) + " CanWeUsePlaceable?: " + IntToString(ai_CanIOpenObject(oCreature, oLocked)) +
    ...
    ai_Debug("0i_associates", "184", "bCanDo: " + IntToString(bCanDo) +
           " CanWeOpenObject?: " + IntToString(ai_CanIOpenObject(oCreature, oLocked)));
...
int ai_StayCloseToMaster(object oCreature, float fDistance = AI_RANGE_PERCEPTION)
{
    ...
    ai_ClearCreatureActions(oCreature);
    ai_Debug("0i_associates", "291", "We are too far away! Move to our master.");
...
void ai_SelectAssociateCommand(object oCreature, object oCommander, int nCommand)
{
    ...
    switch(nCommand)
    {
        ...
        case ASSOCIATE_COMMAND_MASTERGOINGTOBEATTACKED:
        {
            ...
            ai_Debug("0i_associate", "403", GetName(oMaster) + " has been attack by " +
        ...
        {
            ai_Debug("0i_associate", "422", GetName(oMaster) + " has commanded " +
        ...
        {
            ai_Debug("0i_associates", "443", GetName(oMaster) + " has commanded " +
        ...
        {
            ai_Debug("0i_associate", "465", GetName(oMaster) + " has commanded " +
        ...
        {
            ai_Debug("0i_associate", "479", GetName(oMaster) + " has commanded " +
    ...
    if(!ai_GetIsBusy(oCreature))
    {
        ...
        if (!ai_GetIsCharacter(oCommander) && !GetIsEnemy(oCommander, oCreature))
        {
            ...
            {
                ai_Debug("0i_associates", "569", GetName(oCreature) + " receives notice that " +
            ...
            {
                ai_Debug("0i_associates", "578", GetName(oCreature) + " receives notice that " +
            ...
            { // Nothing at the moment.
                ai_Debug("0i_associates", "607", GetName(oCreature) + " receives notice that " +
        ...
        switch(nCommand)
        {
            ...
            {
                ai_Debug("0i_associate", "594", GetName(oMaster) + " has attacked!");
...
void ai_OnDeath(object oCreature, object oKiller)
{
    ...
    if(AI_PERMANENT_ASSOCIATES)
    {
        ...
        while (nCntr < 6)
        {
            ai_Debug("0i_associates", "859", GetName(oAssociate) + " nCntr: " + IntToString(nCntr));
