...
void ai_DoActions(object oCreature, int nForm)
{
    ...
    //if(nDifficulty >= AI_COMBAT_SIMPLE && ai_MoralCheck(oCreature)) return;
    int nSpell;
    object oTarget;
    // DIFFICULT+ -Offensive AOE's and Defensive talents.
    if(nDifficulty >= AI_COMBAT_DIFFICULT && bUseMagic)
    {
    ...
    if(nDifficulty >= AI_COMBAT_SIMPLE && bUseMagic && d100() < 31)
    {
