...
void main()
{
    ...
    //ai_Debug("0e_ch_b_castat", "14", GetName(OBJECT_SELF) + " has been hit by a harmful spell(" +
    ...
    else if(!ai_GetIsBusy(oCreature))
    {
        ...
        if(ai_CanIAttack(oCreature) && ai_GetIsInCombat(oCreature)) ai_DoAssociateCombatRound(oCreature);
    }
}
