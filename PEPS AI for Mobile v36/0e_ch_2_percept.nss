...
void main()
{
    ...
    {
        if(AI_DEBUG) ai_Debug("0e_ch_2_percept", "20", GetName(oCreature) + " sees " +
    ...
    {
        if(AI_DEBUG) ai_Debug("0e_ch_2_percept", "26", GetName(oCreature) + " heard " +
                     GetName(GetLastPerceived()) + " Distance: " +
                     FloatToString(GetDistanceBetween(GetLastPerceived(), oCreature), 0, 2) + ".");
    }
    if(GetLastPerceptionVanished ())
    {
        if(AI_DEBUG) ai_Debug("0e_ch_2_percept", "32", GetName(oCreature) + " lost sight of " +
                     GetName(GetLastPerceived()) + ".");
    }
    // We do nothing on Inaudibles so drop out early!
    if(GetLastPerceptionInaudible())
    {
        if(AI_DEBUG) ai_Debug("0e_ch_2_percept", "38", GetName(oCreature) + " lost sound of " +
                     GetName(GetLastPerceived()) + ".");
        return;
    }
    object oLastPerceived = GetLastPerceived();
    if(AI_DEBUG) ai_Debug("0e_ch_2_percept", "43", "Dead? " + IntToString(GetIsDead(oLastPerceived)) +
    ...
    if(GetLastPerceptionVanished())
    {
        ...
        if(AI_DEBUG) ai_Debug("0e_ch_2_percept", "72", " We saw " + GetName(oLastPerceived) + " disappear!");
        ...
        {
            if(AI_DEBUG) ai_Debug("0e_ch_2_percept", "79", "Is this our target? " +
