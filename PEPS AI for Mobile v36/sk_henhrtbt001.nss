...
void main()
{
    ...
    if (GetHasEffect(EFFECT_TYPE_PETRIFY,OBJECT_SELF))
     {
      // Russian Translation.
      //FloatingTextStringOnCreature("Зарала лишена жизненных сил. У вас в запасе всего пара часов, чтобы вернуть ее к жизни.",oMast);
    ...
    if (GetItemInSlot(INVENTORY_SLOT_HEAD) != OBJECT_INVALID
        ...
        && !GetHasEffect(EFFECT_TYPE_PETRIFY,OBJECT_SELF))
     {
      ...
      // Russian Translation.
      //AssignCommand(OBJECT_SELF,ActionSpeakString("Ой, ненавижу шлемы. От них путаютсї волосы. Не, это не длї менї."));
