        ...
        if(GetIsEnemy(oObject))
        {
               ...
               ai_GetIsInLineOfSight(oCreature, oObject))
            {
                ...
                if(bThreat)
                {
                    ...
                    // We want to deprioratize them as they are hard to hit by
                    // making them disabled but a threat.
                    // Lets attack all of the targets we can see first.
                    else if(fDistance < 3.0)
                    {
                        SetLocalInt(oCreature, AI_ENEMY_SEEN + sCnt, TRUE);
...
int ai_GetDifficulty(object oCreature)
{
    ...
    int nDifficulty = GetLocalInt(oCreature, AI_ENEMY_POWER) - GetLocalInt(oCreature, AI_ALLY_POWER) + 10 + nAdjustment;
...
void ai_SetCreatureAIScript(object oCreature)
{
    ...
    ai_Debug("0i_combat", "3207", GetName(oCreature) + " is setting AI to " + sCombatAI);
