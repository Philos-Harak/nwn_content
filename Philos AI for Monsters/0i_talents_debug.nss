...
int ai_TryHealingTalent(object oCreature, int nInMelee, object oTarget = OBJECT_INVALID)
{
    if(ai_GetAssociateMode(oCreature, AI_MODE_HEALING_OFF)) return FALSE;
    ...
    /*if(GetLevelByClass(CLASS_TYPE_CLERIC, oCreature) > 0)
    {
        ...
        stSpell.oPC = OBJECT_INVALID;
        stSpell.oCaster = oCreature;
        stSpell.oTarget = oTarget;
        stSpell.nClass = CLASS_TYPE_CLERIC;
        stSpell.nLevel = nMaxLevel;
        stSpell.nSlot = 0;
        ai_CastSpontaneousCure(stSpell, TRUE);
        return TRUE;
    } */
...
int ai_UseBuffTalent(object oCreature, int nClass, int nLevel, int nSlot, int nSpell, int nType, object oTarget, object oItem)
{
    if(nType == AI_TALENT_TYPE_SPELL)
    {
        if(Get2DAString("classes", "MemorizesSpells", nClass) == "1")
        {
            ...
            {
                ai_CastMemorizedSpell(oCreature, nClass, nLevel, nSlot, oTarget);
                return TRUE;
            }
        }
        else if(GetSpellUsesLeft(oCreature, nClass, nSpell))
        {
            ai_CastKnownSpell(oCreature, nClass, nSpell, oTarget);
            return TRUE;
        }
    }
    else if(nType == AI_TALENT_TYPE_SP_ABILITY)
...
int ai_UseCreatureItemTalent(object oCreature, json jLevel, json jTalent, string sCategory, int nInMelee, object oTarget = OBJECT_INVALID)
{
    ...
    {
        int nHpLost = GetMaxHitPoints(oTarget) - GetCurrentHitPoints(oTarget);
        int nKitHeals = 10 + GetSkillRank(SKILL_HEAL, oCreature);
        // Only use the kit if a roll of 10 + ranks is less than the damage or
        // they are bloody!
        if(nHpLost < nKitHeals || ai_GetPercHPLoss(oTarget) < AI_HEALTH_BLOODY)
        {
            ai_Debug("0i_talents", "1711", "Using HealersKit nInMelee: " + IntToString(nInMelee));
