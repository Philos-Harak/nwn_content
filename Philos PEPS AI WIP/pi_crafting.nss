...
void main()
{
    ...
    json jRow = JsonArray();
    CreateButton(jRow, "Previous Target", "btn_prev_target", 130.0f, 20.0f);
    CreateTextEditBox(jRow, "plc_hold_bind", "txt_model_name", 16, FALSE, 130.0f, 20.0f);
    CreateButton(jRow, "Next Target", "btn_next_target", 140.0f, 20.0f);
    // Add the row to the column.
    json jCol = JsonArray();
    JsonArrayInsertInplace(jCol, NuiRow(jRow));
    // Row 2 (buttons)********************************************************** 422 / 101
    jRow = JsonArray();
    ...
    int nToken = SetWindow(oPC, jLayout, "plcraftwin", "Crafting",
                 0.0, -1.0, 422.0, 359.0, FALSE, FALSE, FALSE, FALSE, TRUE, "pe_crafting");
    // Set all binds, events, and watches.
    // Row 1
    int nSelected = GetLocalInt(oPC, CRAFT_PART_COLOR);
    NuiSetBind(oPC, nToken, "btn_prev_target", JsonBool(nSelected));
    NuiSetBind(oPC, nToken, "btn_prev_target_event", JsonBool(TRUE));
    NuiSetBindWatch(oPC, nToken, "txt_model_name", TRUE);
    NuiSetBind(oPC, nToken, "txt_model_name", JsonString(GetLocalString(oPC, CRAFT_MODEL)));
    NuiSetBind(oPC, nToken, "btn_next_target", JsonBool(TRUE));
