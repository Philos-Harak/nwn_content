...
void ai_ReactToAlly(object oCreature, object oSpeaker)
{
    ...
    if(AI_DEBUG) ai_Debug("nw_c2_default4", "43", " Distance: " + FloatToString(GetDistanceBetween(oCreature, oTarget), 0, 2));
       ...
       LineOfSightObject(oCreature, oSpeaker))
    {
        ...
        {
            if(AI_DEBUG) ai_Debug("nw_c2_default4", "50", "Moving towards " + GetName(oTarget));
            ActionMoveToObject(oTarget, TRUE, AI_RANGE_CLOSE);
            SetLocalInt(oCreature, AI_AM_I_SEARCHING, TRUE);
        }
        else
        {
            if(AI_DEBUG) ai_Debug("nw_c2_default4", "56", "Searching for " + GetName(oTarget));
...
void ai_MonsterCommands(object oCreature, object oSpeaker, int nMatch)
{
    ...
    {
        if(AI_DEBUG) ai_Debug("nw_c2_default4", "67", GetName(oCreature) + " heard " +
    ...
    {
        if(AI_DEBUG) ai_Debug("nw_c2_default4", "74", GetName(oCreature) + " heard " +
    ...
    {
        if(AI_DEBUG) ai_Debug("nw_c2_default4", "81", GetName(oCreature) + " heard " +
